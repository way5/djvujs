...
const initialState = Object.freeze({
    ...
    options: {
        // all these options are saved in localStorage
    ...
    uiOptions: {
        // aren't saved, should be set programmatically, if required
    ...
    switch (action.type) {
        ...
        case ActionTypes.SET_VIEW_MODE:
            return {
                ...
                viewMode: payload,
                isLoading:
                    payload === Constants.TEXT_MODE ? false : state.isLoading,
            };
        ...
        case Constants.DOCUMENT_CREATED_ACTION:
            return {
                ...
                isLoading: true,
                viewMode: state.options.preferContinuousScroll
                    ? Constants.CONTINUOUS_SCROLL_MODE
        ...
        case Constants.CONTENTS_IS_GOTTEN_ACTION:
            return {
                ...
                isContentsOpened:
                    state.uiOptions.showContentsAutomatically &&
                    !state.appContext.isMobile &&
                    !!action.contents,
            };
...
export const get = {
    ...
    firstRowPageCount: (state) => {
        ...
        return Math.max(
            1,
            Math.min(state.options.firstRowPageCount, get.pageCountInRow(state))
        );
    },
    isContentsOpened: (state) => state.isContentsOpened,
    dictionary: (state) =>
        dictionaries[get.options(state).locale] || dictionaries.en,
    ...
    viewMode: (state) => {
        if (
            state.isIndirect &&
            state.viewMode === Constants.CONTINUOUS_SCROLL_MODE
        ) {
