...
export default function pageReducer(state = initialState, action) {
    ...
    switch (action.type) {
        case Constants.DROP_PAGE_ACTION: {
            ...
            if (newPagesList[index]) {
                // some pages (loaded as "last pages" of the previous saga) can not be in the state, but only in the registry in the saga class
                newPagesList[index] = {
        ...
        case Constants.PAGE_IS_LOADED_ACTION:
            ...
            if (page && page.url) {
                // if it has been already loaded we should avoid unnecessary updates
                return state;
            }
            const newPagesList = [...state.pageList];
        ...
        case Constants.SET_NEW_PAGE_NUMBER_ACTION:
            return {
                ...state,
                ...(state.textPageError || state.imagePageError
                    ? singlePageInitialState
                    : null),
